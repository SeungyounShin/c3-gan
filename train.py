...
class Trainer(object):
    def __init__(self):
        ...
        self.netG, self.netD = load_network(self.num_cls, self.device)
        self.optimizerG, self.optimizerD = define_optimizers(self.netG, self.netD)
        self.CE = nn.CrossEntropyLoss()
        self.eye = torch.eye(self.num_cls).to(self.device)
    ...
    def prepare_code(self):
        rand_z = torch.FloatTensor(self.bs, cfg.GAN.Z_DIM).normal_(0, 1).to(self.device)
        rand_cz = torch.FloatTensor(self.bs, cfg.GAN.CZ_DIM).normal_(0, 1).to(self.device)
        rand_c = torch.zeros(self.bs, self.num_cls).to(self.device)
    ...
    def random_grid(self, size):
        ...
        grid = F.affine_grid(theta, size, align_corners=True).to(self.device)
        return grid
    ...
    def train_D(self):
        ...
        mask = torch.eye(feat.shape[0], dtype=torch.bool).to(self.device)
